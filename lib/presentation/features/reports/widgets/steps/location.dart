...
class _LocationStepState extends State<LocationStep> {
  ...
  Widget? _userMarkerIcon;
  List<Polyline> _barangayPolylines = [];
  bool? _isInsideBarangay;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _loadUserMarker();
      _loadBarangayPolylinesAndCheck();
    });
  }

  Future<void> _loadUserMarker() async {
    try {
      final icon = await MarkerService.instance.getCustomUserMarker(
        context,
        logicalSize: 32.0,
      );
      if (mounted) {
  ...
  void didUpdateWidget(LocationStep oldWidget) {
    ...
    if (widget.userLocation != null &&
      ...
      _mapController.move(widget.userLocation!, _locationZoom);
          ...
          child: hasLocation
              ? FlutterMap(
                  ...
                  children: [
                    ...
                    MarkerLayer(markers: userMarkers, rotate: true),
                    PolylineLayer(polylines: _barangayPolylines),
                  ],
                )
              : Center(
