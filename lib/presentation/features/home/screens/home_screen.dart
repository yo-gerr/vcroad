...
import 'package:vcroad/presentation/features/home/widgets/map_controls.dart';
import 'package:vcroad/presentation/features/home/widgets/marker.dart';
import 'package:vcroad/presentation/features/home/widgets/user_stats.dart';
...
import 'package:vcroad/presentation/features/home/widgets/barangay_user_stats.dart';
...
class _HomeState extends State<Home> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Location is started from AppScreen after onboarding dialogs,
      // or on first explicit user action ("center on me" button).
  ...
  Future<void> _rebuildReportMarkers() async {
    ...
    try {
      ...
      final built = await _markerManager.buildMarkersFromReportData(
        reports,
        context: context,
        logicalSize: info.isMobile ? 36.0 : 44.0,
        onTap: (report) {
          if (!mounted) return;
  ...
  Future<void> _rebuildAdvisoryMarkers() async {
    ...
    try {
      ...
      final built = await _markerManager.buildMarkersFromAdvisories(
        list,
        context: context,
        logicalSize: advisoryLogicalSize,
        onTap: (advisory) {
          if (!mounted) return;
  ...
  Future<void> _prepareMarkersAndUserIcon() async {
    ...
    try {
      final info = context.responsive;
      final categoryLogicalSize = 44.0 * info.markerScale;
      final userLogicalSize = 52.0 * info.markerScale;
  ...
  Future<void> _selectSuggestion(PlaceSuggestion s) async {
    try {
      ...
      if (details.location != null) {
        _mapController.move(details.location!, 16.0);
        return;
      }
    } catch (e, st) {
  ...
  Widget build(BuildContext context) {
    ...
    Widget buildMap({required bool gesturesEnabled}) {
      return ValueListenableBuilder<bool>(
        ...
        builder: (context, interactionsEnabled, child) {
          ...
          return FlutterMap(
            key: ValueKey('map_$_mapRecreateId'),
            mapController: _mapController,
            options: MapOptions(
              initialCenter: _valenzuelaCityCenter,
              initialZoom: 13,
              onMapReady: () {
                _isMapReady = true;
              ...
              interactionOptions: InteractionOptions(
                flags: effectiveGestures ? InteractiveFlag.all : 0,
              ),
            ),
            children: [
              TileLayer(
              ...
              if (_markers.isNotEmpty)
                MarkerLayer(markers: _markers.toList(), rotate: true),
            ],
          );
        },
      );
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      // Re-acquire location on resume without triggering a new permission
      // dialog.  Permission may have been revoked while backgrounded;
      // .start() will silently return if permission is no longer granted.
