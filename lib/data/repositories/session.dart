...
class SessionService {
  ...
  /// Returns the existing active session ID for [uid] from Firestore, or null.
  Future<String?> getExistingSessionId(String uid) async {
    try {
      final snap = await _sessionRef(uid).get();
      if (!snap.exists) return null;
      return snap.data()?['activeSessionId'] as String?;
    } catch (_) {
      return null;
    }
  }

  /// Creates a new session if no active session exists for [uid];
  /// otherwise reuses the existing one. Returns the session ID.
  Future<String> ensureSession(String uid) async {
    final existing = await getExistingSessionId(uid);
    if (existing != null) {
