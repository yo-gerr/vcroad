...
import 'package:vcroad_v2/shared/services/image.dart'; // Add this import
...
class ReportService {
  ...
  /// Upload report media (photo or video) with compression and retry logic
  Future<Map<String, String>> uploadReportMedia({
    required String userId,
    required String reportId,
    required MediaType mediaType,
    required dynamic mediaData, // String path or Uint8List bytes
    int maxRetries = 3,
  }) async {
    ...
    // ensure we end up with bytes for upload to avoid File issues on web
    Uint8List? dataBytes;
    ...
    if (mediaType == MediaType.photo) {
      if (mediaData is Uint8List) {
        ...
        // compress on background isolate
      ...
      // video: if bytes provided use them, else read file as bytes (do NOT block UI)
      if (mediaData is Uint8List) {
        dataBytes = mediaData;
      } else if (mediaData is String) {
        // read file to bytes on background isolate
...
// Top-level helper for compute: compress image file path -> Uint8List
class _CompressArgs {
  final String path;
  final int quality;
  final int maxWidth;
...
// Must be top-level or static function for compute
Future<Uint8List?> _compressImageIsolate(_CompressArgs args) async {
  try {
    // Call the ImageService compressor (no `returnBytes` param).
    final result = await ImageService.compressImageFile(
      args.path,
    ...
    if (kDebugMode) debugPrint('[report] compress isolate error: $e');
    return null;
  }
}

Future<Uint8List?> _readFileAsBytesIsolate(String path) async {
  try {
