...
class AdvisoryExportService {
  ...
  // Set this to your functions / API base that exposes /staticmap
  // e.g. https://us-central1-yourproj.cloudfunctions.net/vcroadApi
  static const String apiBase =
      'https://us-central1-vcroad-a0022.cloudfunctions.net/vcroadApi';
  ...
  Future<Uint8List> generateAdvisoryImageBytes(
    ...
    if (_cachedTemplateImage != null) {
      ...
      final bgPaint = Paint()..color = const ui.Color(0xFF001278);
      canvas.drawRect(
        Rect.fromLTWH(0, 0, finalWidth.toDouble(), finalHeight.toDouble()),
        bgPaint,
      );
    ...
    final borderPaint = Paint()..color = const ui.Color(0xFF001278);
    ...
    final framePaint = Paint()
      ...
      ..color = const ui.Color(0x33001278)
      ..strokeWidth = 1.2;
    canvas.drawRRect(
    ...
    final ui.Color advTypeColor = const ui.Color(0xFFCC2B2B);
    ...
    ui.Paragraph cp(
      ...
      return _buildParagraph(
        ...
        size,
        weight: weight,
        color: color ?? const ui.Color(0xFF001278),
        width: rightW,
    ...
    lines.add(
      cp(
        ...
        color: const ui.Color(0xFF001278),
      ),
    );
    lines.add(
      cp(
    ...
    lines.add(
      cp(
        ...
        rightTextSize,
        weight: ui.FontWeight.w700,
        color: const ui.Color(0xFFCC2B2B),
      ),
    );
  ...
  Future<Uint8List> _fetchStaticMap(
    ...
    // Build request body for your /staticmap route.
    // Prefer computing a bounding box from the advisory polylines so exported
    // image is centered/zoomed to show the affected/alternate routes.
    // advisory.center can still be used as fallback.
    final points = <LatLng>[];
    ...
    double lat = 14.7083, lng = 120.9833; // defaults
    int zoom = 14;

    // If we have polyline points, compute bounds and a zoom that fits them.
    if (points.isNotEmpty) {
      ...
      // Add padding (10%) to bounds so lines are not flush with edges
      ...
      // Estimate zoom that fits longitude span into width.
      // World width in pixels at zoom z = 256 * 2^z. LonDegreesPerPixel = 360 / worldPx.
      // Find highest zoom such that required pixel width <= available width.
      final lonSpan = (maxLng - minLng).abs();
      final latSpan = (maxLat - minLat).abs();
      const int maxZoom = 18;
      const int minZoom = 3;
      final paddingPx = (width * 0.08).toInt(); // keep ~8% side padding
      ...
      for (int z = maxZoom; z >= minZoom; z--) {
        ...
        final lonDegPerPx = 360 / worldPx;
        final latDegPerPx = 360 / worldPx; // approximate, OK for our use
        final reqPxLon = (lonSpan / lonDegPerPx).abs();
        final reqPxLat = (latSpan / latDegPerPx).abs();
      ...
      // fallback to advisory.center if available
      ...
      if (c != null) {
        try {
          if (c is Map) {
            ...
            final dyn = c;
            try {
              final maybeLat = dyn.latitude ?? dyn.lat ?? dyn.position?.lat;
              final maybeLng =
                  dyn.longitude ?? dyn.lng ?? dyn.lon ?? dyn.position?.lng;
              if (maybeLat is num) lat = maybeLat.toDouble();
              if (maybeLng is num) lng = maybeLng.toDouble();
            } catch (_) {}
          }
    ...
    // Build body: include paths for affected + alternate roads so static map draws them.
    // Decimate long polylines to reduce request size.
    List<Map<String, dynamic>> buildPaths() {
      final result = <Map<String, dynamic>>[];

      // Scale stroke weight based on the requested map width so lines read as
      // road widths on different output sizes. Keep within reasonable bounds.
      final double scale = math.max(0.8, math.min(2.0, width / 600.0));
      ...
      void addPaths(
        ...
        for (final poly in list) {
          ...
          final maxPoints = 200;
          final step = (poly.length / maxPoints).ceil().clamp(1, poly.length);
          final sampled = <Map<String, double>>[];
          for (int i = 0; i < poly.length; i += step) {
            sampled.add({'lat': poly[i].latitude, 'lng': poly[i].longitude});
          }
          // ensure last point included
          if (sampled.isNotEmpty) {
            final last = poly.last;
            final lastMap = {'lat': last.latitude, 'lng': last.longitude};
            if (sampled.last['lat'] != lastMap['lat'] ||
                sampled.last['lng'] != lastMap['lng']) {
              sampled.add(lastMap);
            }
          }

          // compute scaled weight and clamp
          int computedWeight = (weight * scale).round();
          if (computedWeight < 2) computedWeight = 2;
          if (computedWeight > 20) computedWeight = 20;

          result.add({
            // use hex colors for predictable rendering in Google Static Maps
            'color': color,
      ...
      // Affected: thicker red; Alternate: blue and slightly thinner but still visible.
      addPaths(advisory.affectedRoads, '0xCC0000', 8, dashed: false);
      addPaths(advisory.alternateRoutes, '0x0066CC', 6, dashed: true);
      return result;
    }
    ...
    final url = Uri.parse('$apiBase/staticmap');
    ...
    // fallback: return a tiny blank PNG encoded asset
    final fallback = await rootBundle
        .load('assets/images/blank_map_fallback.png')
  ...
  ui.Paragraph _buildParagraph(
    ...
    ui.Color color = const ui.Color(0xFF111111),
    double width = 400,
