...
import 'package:vcroad_v2/shared/services/lesson_image.dart';
import 'dart:math';
...
class LessonService {
  ...
  final LessonImageService _imageService = LessonImageService.instance;
  ...
  Future<void> updateLesson(QuizMaterial lesson) async {
    try {
      ...
      // Read existing lesson from Firestore so we can detect removed remote urls
      QuizMaterial? existing;
      try {
        ...
        if (doc.exists && doc.data() != null) {
          existing = QuizMaterial.fromJson(doc.data()!);
        }
      } catch (e) {
        debugPrint(
          'LessonService.updateLesson: failed to fetch existing lesson: $e',
        );
      }

      // Upload any new images for the updated lesson — this returns lesson with remote urls
      final uploadedLesson = await _imageService.uploadLessonImages(
        lessonWithAudit,
      );

      // If we had an existing lesson, determine which remote urls were removed and delete them.
      if (existing != null) {
        final oldUrls = _imageService.collectRemoteUrlsFromLesson(existing);
        final newUrls = _imageService.collectRemoteUrlsFromLesson(
          uploadedLesson,
        );
        ...
        if (toDelete.isNotEmpty) {
          await _imageService.deleteRemoteFilesByUrls(toDelete);
        }
      }

      // Persist the uploaded lesson (with remote urls)
      await _firestore
          .collection(_collection)
          .doc(uploadedLesson.id)
          .set(uploadedLesson.toJson());
    } catch (e) {
